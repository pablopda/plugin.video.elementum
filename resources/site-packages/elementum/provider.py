...
def register(search, search_movie, search_episode, search_season=None):
    try:
        ...
        if "method" not in payload or "search_object" not in payload:
            raise ValueError("Payload missing required fields: method, search_object")
    except (ValueError, TypeError) as e:
        log.error("Failed to decode provider payload: %s" % e)
    ...
    try:
        ...
        try:
            ...
            notify(py2_encode("%s: %s" % (getElementumLocalizedString(30224), repr(e)), 'utf-8'))
            try:
                with urllib_request.urlopen("%s/provider/%s/failure" % (ELEMENTUMD_HOST, ADDON_ID)) as response:
                    pass
            except Exception as failure_e:
                log.debug("Failed to report provider failure: %s" % failure_e)
                pass
    finally:
        try:
