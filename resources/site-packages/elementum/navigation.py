...
def getInfoLabels():
    ...
    try:
        with closing(urllib_request.urlopen(url)) as response:
            ...
            return resolved
    except (urllib_error.URLError, urllib_error.HTTPError, socket.timeout, ValueError, KeyError) as e:
        log.debug("Could not resolve TMDB item %s: %s" % (tmdb_id, repr(e)))
        return {}
...
def _json(url):
    with closing(urllib_request.urlopen(url)) as response:
        ...
        if response.code == 300:
            ...
            try:
                player = xbmc.Player()
                if player.isPlaying() and ADDON_ID in player.getPlayingFile():
                    player.pause()
            except Exception as e:
                log.debug("Could not pause player: %s" % repr(e))
...
def run(url_suffix="", retry=0):
    ...
    try:
        ...
        log.debug("Could not parse buffer_timeout setting, using default: %s" % repr(e))
        buffer_timeout = 60
    ...
    try:
        ...
        log.debug("Could not parse preload_timeout setting, using default: %s" % repr(e))
        preload_timeout = 1
    ...
    if login or password:
        log.info("Setting auth to %s:***" % (login,))
    ...
    try:
        ...
        except (TypeError, LookupError, UnicodeError) as text_err:
            log.debug("Could not convert error to text: %s" % repr(text_err))
            try:
                msg = six.ensure_binary(e.__str__(), errors='ignore')
            except (TypeError, LookupError, UnicodeError) as binary_err:
                log.debug("Could not convert error to binary: %s" % repr(binary_err))
                msg = repr(e)
