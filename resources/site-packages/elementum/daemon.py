...
try:
    ...
    log.warning("Cannot import subprocess module: %s" % e)
    hasSubprocess = False
...
# Lock for thread-safe access to global state (lib, last_lib, last_exit_code)
_daemon_lock = threading.Lock()

def sanitize_args_for_logging(args):
    """Sanitize command arguments to mask sensitive data like passwords."""
    import re
    if isinstance(args, list):
        sanitized = []
        for arg in args:
            if isinstance(arg, str) and '-localPassword=' in arg:
                sanitized.append('-localPassword=***')
            else:
                sanitized.append(arg)
        return sanitized
    elif isinstance(args, str):
        return re.sub(r'-localPassword=[^\s]+', '-localPassword=***', args)
    return args
...
def get_addon_profile_dir():
    ...
    if isinstance(profile_path, bytes):
        return profile_path.decode('utf-8')
    return profile_path
...
def get_elementum_binary():
    ...
    for file_ in (dest_files - orig_files):
        ...
        try:
            ...
            # File was already deleted by another process (race condition)
            pass
        except OSError as e:
...
def clear_fd_inherit_flags():
    ...
    try:
        ...
        log.warning("Failed to clear file descriptor inherit flags: %s" % e)
...
def start_elementumd(monitor, **kwargs):
    ...
    # Cache settings once to avoid repeated getSetting calls (performance optimization)
    local_port = ADDON.getSetting("local_port")
    remote_host = ADDON.getSetting("remote_host")
    remote_port = ADDON.getSetting("remote_port")
    ...
    if local_port != "":
        ...
        shared_args += " -remotePort=" + local_port
    ...
    # Create environment for both platforms
    env = os.environ.copy()

    # Pass password via environment variable instead of command-line args for security.
    # NOTE: This requires the Elementum binary to support the -localPasswordEnv flag,
    # which tells it to read the password from the specified environment variable.
    # If the binary doesn't support this flag, revert to using -localPassword=<value>
    # directly in args (less secure as password is visible in process list).
    if remote_password != "":
        env["ELEMENTUM_LOCAL_PASSWORD"] = remote_password
        args.append("-localPasswordEnv=ELEMENTUM_LOCAL_PASSWORD")
        shared_args += " -localPasswordEnv=ELEMENTUM_LOCAL_PASSWORD"
    ...
    if not proc and ctypes_is_supported:
        try:
            ...
            try:
                ...
                # Use 'w' mode to atomically create or truncate the file
                # This avoids TOCTOU race condition with exists() check
            ...
            library_thread = threading.Thread(target=start_library, args=[monitor, lib_to_use, log_path, shared_args, library_path])
...
def start_library(monitor, lib, log_path, args, library_path):
    ...
    log.info("Preparing start with args '%s' and log path: %s" % (sanitize_args_for_logging(args), log_path))

    lib.startWithLogAndArgs.argtypes = [c_char_p, c_char_p]
    lib.startWithLogAndArgs.restype = c_int

    exit_code = lib.startWithLogAndArgs(c_char_p(log_path.encode('utf-8')), c_char_p(args.encode('utf-8')))
    with _daemon_lock:
        last_exit_code = exit_code
    log.info("Shared library execution stopped with code %d" % (exit_code))
...
def shutdown():
    try:
        with urllib_request.urlopen(ELEMENTUMD_HOST + "/shutdown", timeout=1) as response:
            pass
    except Exception:
        pass
        ...
        while not monitor_abort.abortRequested():
            ...
            if restart_count > max_restart or last_code == -9:
                if monitor.reboot():
                    ...
                    # Use waitForAbort for early exit instead of blocking sleep
                    if monitor_abort.waitForAbort(1):
                        break
            ...
            if proc is None:
                ...
                if isinstance(proc, threading.Thread):
                    while not os.path.exists(log_path):
                        # Use waitForAbort for early exit instead of blocking sleep
                        if monitor_abort.waitForAbort(0.5):
                            break
            ...
            # Use waitForAbort for early exit instead of blocking sleep
            if monitor_abort.waitForAbort(3):
                break
...
def is_writable(path):
    try:
        ...
        log.warning("Path %s is not writable: %s" % (path, e))
        return False
    return True
