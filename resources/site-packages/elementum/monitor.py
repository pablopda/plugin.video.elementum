...
class ElementumMonitor(Monitor):
    ...
    def restart(self):
        try:
            ...
            init()
            with urllib_request.urlopen("%s/restart" % ELEMENTUMD_HOST, timeout=1) as response:
                pass
        except Exception as e:
            log.debug("Restart request failed (expected on timeout): %s" % e)
            pass
    ...
    def onSettingsChanged(self):
        try:
            ...
            init()
            with urllib_request.urlopen("%s/reload" % ELEMENTUMD_HOST, timeout=1) as response:
                pass
            with urllib_request.urlopen("%s/cmd/clear_page_cache" % ELEMENTUMD_HOST, timeout=1) as response:
                pass
        except Exception as e:
            log.debug("Settings change notification failed: %s" % e)
            pass
    ...
    def onNotification(self, sender, method, data):
        ...
        try:
            ...
            with urllib_request.urlopen("%s/notification?sender=%s&method=%s&data=%s" % (
                ELEMENTUMD_HOST,
                quote(sender, safe=''),
                quote(method, safe=''),
                quote(data_base64, safe=''))) as response:
                pass
        except Exception as e:
            log.debug("Notification forwarding failed: %s" % e)
            pass
