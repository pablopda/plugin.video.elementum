...
class ElementumRPCServer(BaseHandler):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Instance variables to prevent memory leaks across multiple RPC server instances
        self._objects = {}
        self._failures = {}
        self._objects_lock = threading.RLock()
        self._failures_lock = threading.RLock()
    ...
    def Reset(self):
        with self._objects_lock:
            for i in list(self._objects.keys()):
                obj = self._objects[i]
                try:
                    # Try to close dialog objects properly
    ...
    def Dialog_Text(self, title, text):
        ...
        while (retry > 0):
            try:
                ...
                log.debug("Failed to set dialog text controls: %s", repr(e))
                return
    ...
    def Player_GetSubtitles(self):
        try:
            ...
            log.debug("Failed to get available subtitle streams: %s", repr(e))
    ...
    def AddonFailure(self, addonId):
        ...
        with self._failures_lock:
            ...
            failure_count = self._failures[addonId]

            # Prevent unbounded growth - keep only last 100 entries
            if len(self._failures) > 100:
                oldest_keys = list(self._failures.keys())[:50]
                for k in oldest_keys:
                    del self._failures[k]

        log.warning("Recorded failure %d for %s" % (failure_count, addonId))

        try:
            allowed_failures = int(ADDON.getSetting("provider_allowed_failures"))
        ...
        if failure_count > allowed_failures:
            try:
                ...
                with urllib_request.urlopen("%s/provider/%s/disable" % (ELEMENTUMD_HOST, addonId)) as response:
                    pass
            except (IOError, OSError, Exception) as e:
                log.error("Failed to disable provider %s: %s", addonId, repr(e))
    ...
    def GetCurrentView(self):
        ...
        try:
            ...
            log.debug("Could not find defaultresolution in skin, trying alternative pattern: %s", repr(e))
    ...
    def DialogProgress_Create(self, *args, **kwargs):
        ...
        with self._objects_lock:
            ...
            if len(self._objects) > 100:
                oldest_keys = list(self._objects.keys())[:50]
                for k in oldest_keys:
    ...
    def DialogProgress_IsCanceled(self, hwnd, *args, **kwargs):
        with self._objects_lock:
            dialog = self._objects[hwnd]
        if PLATFORM['kodi'] < 19:
            return int(dialog.iscanceled(*args, **kwargs))
        else:
            heading, message = makeMessage(*args, **kwargs)
            return int(dialog.iscanceled(heading, message))

    def DialogProgress_Update(self, hwnd, *args, **kwargs):
        with self._objects_lock:
            dialog = self._objects[hwnd]
        if PLATFORM['kodi'] < 19:
            return dialog.update(*args, **kwargs)
        else:
            heading, message = makeMessage(*args, **kwargs)
            return dialog.update(heading, message)
    ...
    def DialogProgressBG_Update(self, hwnd, percent, heading, message):
        with self._objects_lock:
            ...
            dialog = self._objects[hwnd]
        return dialog.update(percent, heading, message)
